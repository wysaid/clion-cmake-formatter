...
- **缩进大小 (1-16)**：支持紧凑（1-2 空格）和宽松（8-16 空格）的风格。超出此范围的值会被限制到最近的边界。
- **行长度 (0 或 ≥30)**：0 表示不限制；非零值小于 30 会被设置为 30，以防止过度换行导致基本命令都无法阅读。
- **空行数 (0-20)**：防止意外的过多空白。超过 20 个连续空行很少是有意为之的。

当检测到无效值时，会自动修正为最接近的有效值，并显示警告消息说明修正内容。
